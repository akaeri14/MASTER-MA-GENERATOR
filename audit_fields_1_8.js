...
while ((match = regex.exec(docXml)) !== null && count < 8) {
  count++;
  ...
  console.log(`Is begin inside paragraph? ${beginIndex >= pStart && beginIndex <= pEnd}`);
  console.log(`Is separate inside paragraph? ${sepIndex >= pStart && sepIndex <= pEnd}`);
  console.log(`Is end inside paragraph? ${endIndex >= pStart && endIndex <= pEnd}`);
  console.log(`Paragraph XML:\n${pXml}`);
}
